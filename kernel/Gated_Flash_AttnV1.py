...
def test_close():
    ...
    G_in = torch.randn(B, H, S, 1, device=device, requires_grad=False, dtype=torch.float16)
    G_out = torch.randn(B, H, S, 1, device=device, requires_grad=False, dtype=torch.float16)
    scale = 1/math.sqrt(D)
